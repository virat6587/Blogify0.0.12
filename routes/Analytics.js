...
router.get("/trending", async (req, res) => {
  try {
    ...
    const sanitized = blogs.map(b => ({
      title: b.blog?.title, slug: b.blog?.slug, coverImageURL: b.blog?.coverImageURL,
      authorName: b.blog?.createdBy?.fullName, totalViews: b.totalViews, totalLikes: b.totalLikes, totalComments: b.totalComments
    }));
    res.json({ success: true, blogs: sanitized });
  } catch (error) { console.error("Trending error:", error); res.status(500).json({ success: false, message: "Failed to fetch trending blogs" }); }
});
...
router.get("/most-liked", async (req, res) => {
  try {
    ...
    const sanitized = blogs.map(b => ({
      title: b.blog?.title, slug: b.blog?.slug, coverImageURL: b.blog?.coverImageURL,
      authorName: b.blog?.createdBy?.fullName, totalViews: b.totalViews, totalLikes: b.totalLikes
    }));
    res.json({ success: true, blogs: sanitized });
  } catch (error) { console.error("Most liked error:", error); res.status(500).json({ success: false, message: "Failed to fetch most liked blogs" }); }
});

router.get("/blog/:blogId", restrictToLoggedInUserOnly, async (req, res) => {
  try {
    const { blogId } = req.params;
    if (!validateObjectId(blogId)) return res.status(400).json({ success: false, message: "Invalid blog ID" });
  ...
  } catch (error) { console.error("Blog analytics error:", error); res.status(500).json({ success: false, message: "Failed to fetch blog analytics" }); }
});
...
router.get("/author/stats", restrictToLoggedInUserOnly, async (req, res) => {
  ...
  } catch (error) { console.error("Author stats error:", error); res.status(500).json({ success: false, message: "Failed to fetch author analytics" }); }
});
...
router.get("/admin/dashboard", restrictToLoggedInUserOnly, restrictTo(["ADMIN"]), async (req, res) => {
  try {
    ...
    res.json({ success: true, stats: { totalUsers, totalBlogs, totalComments, platform: platformStats[0] || {} } });
  } catch (error) { console.error("Admin analytics error:", error); res.status(500).json({ success: false, message: "Failed to fetch admin analytics" }); }
});
