...
const { followLimiter } = require("../middlewares/rateLimiting");
const { validateObjectId } = require("../middlewares/validation");
const NotificationService = require("../services/notificationService");

router.use(restrictToLoggedInUserOnly);

router.post("/:userId/follow", followLimiter, async (req, res) => {
  try {
    const { userId } = req.params;
    if (!validateObjectId(userId)) return res.status(400).json({ success: false, message: "Invalid user ID" });
    const currentUserId = req.user._id.toString();
    if (userId === currentUserId) return res.status(400).json({ success: false, message: "Cannot follow yourself" });
  ...
  } catch (error) { console.error("Follow Error:", error); res.status(500).json({ success: false, message: error.message || "Failed to follow user" }); }
});
...
router.get("/:userId/followers", async (req, res) => {
  ...
  } catch (error) { console.error("Error fetching followers:", error); res.status(500).json({ success: false, message: "Failed to fetch followers" }); }
});
...
router.get("/:userId/following", async (req, res) => {
  ...
  } catch (error) { console.error("Error fetching following:", error); res.status(500).json({ success: false, message: "Failed to fetch following" }); }
});
