...
router.post("/add-new", blogCreationLimiter, cloudinaryUpload.single("coverImage"), async (req, res) => {
  try {
    ...
    res.render("addBlog", { user: req.user, error: "Something went wrong while creating the blog." });
  }
});

// GET edit blog form (ownership check)
...
// GET single blog (public read, but auth for likes)
...
// PUT update blog (ownership check)
router.put("/:id", cloudinaryUpload.single("coverImage"), async (req, res) => {
  try {
    ...
    const updatedBlog = await Blog.findById(req.params.id).populate("createdBy", "fullName profileImageURL bio followers").lean();
...
router.get("/tags/:tag", async (req, res) => {
  try {
    ...
    console.error("Tagged blogs error:", error);
    res.status(500).send("Internal Server Error");
  }
});
