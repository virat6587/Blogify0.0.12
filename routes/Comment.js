...
router.get("/blog/:blogId", async (req, res) => {
  try {
    ...
    const limit = 10, skip = (parseInt(page) - 1) * limit;
  ...
  } catch (error) { console.error("Error fetching comments:", error); res.status(500).json({ success: false, message: "Failed to fetch comments" }); }
});

router.post("/blog/:blogId", commentLimiter, async (req, res) => {
  ...
  try {
    ...
    if (parentCommentId && !validateObjectId(parentCommentId)) return res.status(400).json({ success: false, message: "Invalid parent comment ID" });
...
router.put("/:commentId", async (req, res) => {
  try {
    ...
    if (!validateObjectId(commentId)) return res.status(400).json({ success: false, message: "Invalid comment ID" });
    ...
    comment.content = content.trim().substring(0, 5000);
    await comment.save();
    res.json({ success: true, comment });
  } catch (error) { console.error("Error updating comment:", error); res.status(500).json({ success: false, message: "Failed to update comment" }); }
});
...
router.delete("/:commentId", async (req, res) => {
  ...
  } catch (error) { console.error("Error deleting comment:", error); res.status(500).json({ success: false, message: "Failed to delete comment" }); }
});

router.post("/:commentId/like", likeLimiter, async (req, res) => {
  try {
    const { commentId } = req.params;
    if (!validateObjectId(commentId)) return res.status(400).json({ success: false, message: "Invalid comment ID" });
    const comment = await Comment.findById(commentId);
  ...
  } catch (error) { console.error("Error liking comment:", error); res.status(500).json({ success: false, message: "Failed to like comment" }); }
});
